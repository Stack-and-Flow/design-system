...
## Environment validation (AI agents only)

> ⚠️ This section is for AI agents (opencode + gentle-ai). Human contributors can ignore it.

Before starting implementation on any `.ts`/`.tsx` file, **verify the contributor's VS Code environment** for formatter conflicts:

1. **Read the user's VS Code settings**: check `%APPDATA%\Code\User\settings.json` for any `[javascript]`, `[typescript]`, `[javascriptreact]`, `[typescriptreact]` sections with `editor.defaultFormatter` set to `esbenp.prettier-vscode` or `vscode.typescript-language-features`.

2. **If a conflict is detected**: show a single clear warning at the start:
   ```
   ⚠️ Formatter conflict detected — your VS Code user settings have
   Prettier/TS formatter set as default for JS/TS files. This project
   uses Biome. The workspace settings already configure Biome as the
   formatter, but your user-level settings may override it.
   
   Fix: remove `editor.defaultFormatter` from the matching language
   sections in your user settings.json, or set them to `biomejs.biome`.
   ```

3. **Do NOT modify the user's settings file** — only warn. The fix is the user's responsibility.
