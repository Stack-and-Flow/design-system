...
- When a component needs an actual loading spinner, prefer `SpinnerCircular` from `spinners-react` for consistency with `Button`; token-backed CSS loaders are acceptable only for structural/non-control loading states where the component already owns a custom loading surface.
