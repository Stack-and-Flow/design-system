...
export const MonthYearPickerDropdown: React.FC<MonthYearPickerDropdownProps> = ({
  ...
  return (
    <div className='absolute inset-0 z-10 flex flex-col bg-background-light dark:bg-surface-dark rounded-md animate-fadeIn w-full h-full'>
      <div className='flex-1 flex flex-col justify-center items-center w-full h-full p-0'>
          ...
          >
            {monthNames.map((month, idx) => (
              <button
                ...
                className={`w-full py-1.5 px-3 rounded-md font-semibold text-base text-left transition-colors duration-150 ${idx === selectedMonth ? 'bg-primary text-text-dark dark:bg-primary dark:text-text-dark' : 'hover:bg-surface-raised-light dark:hover:bg-white-tint-faint text-text-light dark:text-text-dark'} ${isMonthDisabled(selectedYear, idx) ? 'opacity-50 cursor-not-allowed' : ''}`}
          ...
          >
            {years.map((year) => (
              <button
                ...
                className={`w-full py-1.5 px-3 rounded-md font-semibold text-base text-left transition-colors duration-150 ${year === selectedYear ? 'bg-primary text-text-dark dark:bg-primary dark:text-text-dark' : 'hover:bg-surface-raised-light dark:hover:bg-white-tint-faint text-text-light dark:text-text-dark'}`}
                style={{ letterSpacing: '0.01em', fontSize: '0.95rem' }}
        ...
        <div className='flex justify-end p-2 gap-2'>
          <button
            className='px-2 py-1 rounded-md bg-surface-raised-light text-text-light hover:bg-surface-raised-dark dark:bg-surface-raised-dark dark:text-text-dark dark:hover:bg-surface-raised-dark text-sm transition-colors duration-150'
            onClick={onCancel}
          >
            {labels.cancel}
          </button>
          <button
            className='px-2 py-1 rounded-md bg-primary text-text-dark hover:bg-primary-hover dark:bg-primary dark:text-text-dark dark:hover:bg-primary-hover text-sm transition-colors duration-150'
